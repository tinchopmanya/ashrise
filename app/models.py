...
class RadarCandidateLink(Base):
    __tablename__ = "radar_candidate_links"
    __table_args__ = (
        CheckConstraint(
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, server_default=func.uuid_generate_v4())
    radar_candidate_id: Mapped[str] = mapped_column(
        UUID(as_uuid=False),
...
class RadarPromotion(Base):
    __tablename__ = "radar_promotions"
    __table_args__ = (
        CheckConstraint(
    ...
    status: Mapped[str] = mapped_column(String(32), nullable=False, server_default=text("'previewed'"))
    target_type: Mapped[str] = mapped_column(String(64), nullable=False, server_default=text("'vertical_candidate'"))
    target_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    payload_snapshot: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default=text("'{}'::jsonb"))
